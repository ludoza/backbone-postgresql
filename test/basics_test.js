...
Backbone.pg_connector.config = {
    db: 'pg://bb_pg:test@localhost/bb_pg_test'
};
    ...
    describe('on models', function (done) {
        ...
        describe('saving a new model', function (done) {
            ...
            it('should not save invalid columns', function (done) {
                ...
                test_model.has_attributes = function () {
                    return false;
                }
                test_model.save(null, {
                    success: function (model) {
                        var test_model2 = new Test({
                            id: model.id
                        });
                        test_model2.fetch({
                            success: function (model2) {
                                model2.attributes.should.eql({
                                    id: model.id,
                                    one: 'one',
                                    two: 2
                                });
    ...
    describe('on collections', function (done) {
        describe("fetching a collection", function (done) {
            ...
            beforeEach(function (done) {
                ...
                test_model1 = new Test({
                    one: 'testone1',
                    two: 2
                });
                test_model2 = new Test({
                    one: 'testone2',
                    two: 2
                });
                test_model1.save(null, {
                    success: function () {
                        test_model2.save(null, {
                            success: function () {
                                done();
                            }
                        });
                ...
                collection.fetch({
                    success: function () {
                        ...
                        collection.map(function (x) {
                            return x.id
                        }).sort().should.eql([test_model1.id, test_model2.id].sort());
                        done();
                    }
                });
            });
            ...
            it('should filter the collection with multiple conditions in an array', function (done) {
                var test_model3 = new Test({
                    one: 'testone1',
                    two: 3
                });
                test_model3.save(null, {
                    success: function () {
